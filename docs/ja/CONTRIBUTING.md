...
**再起動で実際に変わるコミットだけを数えます。** 再起動は実行中のセッションをすべて殺すので、
再起動を要求するアラートはそれに見合うものでなければなりません。見合わなければ運用者は再起動を
見送り、検査は何日も赤のまま放置され、やがて誰も読まなくなります。`docs/`・`tests/`・
`examples/`・`.github/`・`LICENSE`・トップレベルの `*.md` しか触らないコミットは、ボットの
挙動を1ビットも変えられません。`uv.lock` だけを変更し、更新した依存をすべて
`dependency-type: direct:development` と表明している Dependabot のバンプも同様です。
どちらも数えず、報告する経過日数は**再起動が必要なコミットのうち最も古いもの**から求めます。
それ以外はすべて数えます。変更ファイル一覧を読み取れなかったコミットも数えます。分類できない
ものは「数える」でなければならず、「無害と見なす」にしてはいけません。
...
- **自動パッチバンプ**: `main` にマージされた PR ごとにパッチバージョンが自動的にインクリメントされます（例: `1.3.0` → `1.3.1`）。リリースタグは作成されません。バンプは `main` へ直接コミットされません — `issue-driven-main` ルールセットは bypass リストが空なため、ワークフローが `auto/bump-v<version>` ブランチの PR を開き（ルールセットがすべての PR に Issue の紐付けを要求するため、専用の Issue も作成します）、その PR を他と同じようにマージします。バンプ PR がすでに開いている場合、その間にマージされた変更は 2 つ目の PR を作らず、その PR にまとめられます。
- **手動マイナー/メジャーリリース**: `1.4.0` などのマイナー/メジャーリリースを切る場合は、`pyproject.toml` と `CHANGELOG.md` を手動で更新し、`uv lock` を実行して `uv.lock` の editable エントリを同じバージョンに揃えた上で、PR タイトルに `[release]` を含めます。これによりパッチバンプなしで現在のバージョンが GitHub Release としてタグ付け・公開されます。

...
- `/effort [level] [scope]` — show or switch the **reasoning effort** used by the current backend. Valid levels are backend-specific: Claude accepts `low/medium/high/max`; Codex accepts `low/medium/high/xhigh/max/ultra` (mapped to the CLI's `model_reasoning_effort`). Leave it unset to defer to the CLI default.

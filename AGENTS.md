...
17. **A backend that cannot offer a control says so, rather than pretending** (`claude_code_core/pi_runner.py`): the pi CLI has no sandbox and shows no trust prompt in the non-interactive modes ccdb uses, so there is no equivalent of `permission_mode` to configure — unsandboxed is the only mode pi has, and `--tools` is the only restriction left. `/backend pi` therefore refuses to spawn until `CCDB_PI_ALLOW_UNSANDBOXED=1` is set in the deployment environment (never per thread, or any Discord user could grant it to themselves), and declines project trust with `--no-approve` unless `CCDB_PI_APPROVE_PROJECT=1` — a repository ccdb checks out must not be able to reconfigure the agent about to run inside it. The parser is built from *recorded* runs for the same reason: pi's published event list says a failed turn is an `error` event, and the measured truth is an assistant `message_end` with `stopReason: "error"` and exit code 0, which a trusting parser renders as a successful empty answer. Same for the terminal event (`agent_settled`, undocumented) and usage (per message, not per turn). See `docs/pi-backend.md` and ADR-0007.
